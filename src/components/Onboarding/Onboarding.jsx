...
function Onboarding() {
  ...
  const handleDocumentUpload = async (e, index) => {
    ...
    // Check if this is a passport photo upload
    const isPassportPhoto =
      formData.documents[index].type === "Latest Passport Photo - Scan Copy";

    // Get user info early for passport photo uploads
    const user = isPassportPhoto
    ...
    try {
      ...
      if (isPassportPhoto) {
        // Use special endpoint for passport photos
        if (!user?.employeeId) {
          throw new Error("Employee ID not found. Please log in again.");
        }

        formDataToSend.append("employeeId", user.employeeId);
        response = await fetch(getApiUrl("/api/upload-passport-photo"), {
          method: "POST",
          body: formDataToSend,
        });
      } else {
        // Use regular upload endpoint for other documents
        ...
        documents: prev.documents.map((doc, i) =>
          i === index
            ? {
                ...
                file: null,
                uploading: false,
                // Store additional info for passport photos
                ...(isPassportPhoto && {
                  employeePhotoUrl: data.employeePhotoUrl,
                  employeePhotoKey: data.employeePhotoKey,
      ...
      if (isPassportPhoto) {
        console.log(
          `✅ Passport photo uploaded successfully for employee ${user.employeeId}`
        );
        console.log(`📤 Onboarding URL: ${data.url}`);
        console.log(`👤 Employee Photos URL: ${data.employeePhotoUrl}`);
      }
    } catch (err) {
  ...
  const handleDocumentRemove = async (index) => {
    ...
    const isPassportPhoto = doc.type === "Latest Passport Photo - Scan Copy";
    ...
    if (doc.url && doc.key) {
      ...
      try {
        if (isPassportPhoto && doc.employeePhotoKey) {
          // Delete from both buckets for passport photos
          const deletePromises = [
            fetch(getApiUrl("/api/upload"), {
              method: "DELETE",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({ key: doc.key, bucket: "uploads" }),
            }),
            fetch(getApiUrl("/api/upload"), {
              method: "DELETE",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({
                key: doc.employeePhotoKey,
                bucket: "faces",
              }),
            }),
          ];
          ...
          if (failedResponses.length > 0) {
            throw new Error("Failed to delete from one or both buckets");
          }

          console.log(
            `🗑️ Passport photo deleted from both buckets for employee`
          );
        } else {
          // Delete from single bucket for other documents
        ...
        alert(`Failed to delete file from S3: ${err.message}`);
      }
    } else {
      setFormData((prev) => ({
        ...prev,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Include all documents with URLs (both existing and newly uploaded)
      dataToSubmit.documents = documents
        ...
        .map((doc) => ({
          ...
          url: doc.url,
          key: doc.key,
          // Include additional fields for passport photos
          ...(doc.employeePhotoUrl && {
            employeePhotoUrl: doc.employeePhotoUrl,
          }),
          ...(doc.employeePhotoKey && {
            employeePhotoKey: doc.employeePhotoKey,
          }),
  ...
  // Auto-populate form if onboarding record or login record exists
  useEffect(() => {
    async function fetchOnboardingData() {
      ...
      try {
        ...
        const res = await fetch(
          getApiUrl(
            `/api/employee/onboarding-data?employeeId=${user.employeeId || ""}`
          ),
          {
        ...
        if (res.ok) {
          ...
          [
            ...
            if (formattedData[field]) {
              const d = new Date(formattedData[field]);
              if (!isNaN(d)) {
                formattedData[field] = d.toISOString().split("T")[0];
              }
            }
          });
          // Merge: only overwrite fields in formData that are empty
          setFormData((prev) => {
            const merged = { ...prev };
            Object.keys(formattedData).forEach((key) => {
              if (key === "documents") {
                // Special handling for documents array - merge existing documents with uploaded ones
                const existingDocs = formattedData.documents || [];
                const currentDocs = merged.documents || [];

                // Create a map of existing documents by type
                const existingDocsMap = {};
                existingDocs.forEach((doc) => {
                  existingDocsMap[doc.type] = doc;
                });

                // Merge documents, preserving uploaded files
                const mergedDocs = currentDocs.map((doc) => {
                  const existingDoc = existingDocsMap[doc.type];
                  if (existingDoc && existingDoc.url) {
                    // If we have an existing uploaded document, use it
                    return {
                      ...doc,
                      url: existingDoc.url,
                      key: existingDoc.key,
                      employeePhotoUrl: existingDoc.employeePhotoUrl,
                      employeePhotoKey: existingDoc.employeePhotoKey,
  ...
  useEffect(() => {
    async function fetchUserStatus() {
      ...
      try {
        const res = await fetch(
          getApiUrl(
            `/api/employee/info?employeeId=${user.employeeId || ""}&username=${
              user.username || ""
            }`
          )
        );
              ...
              {formData.documents.map((doc, index) => (
                <div key={index} className="document-item">
                  <label className="document-label">
                    {doc.type}
                    {doc.type === "All Experience Letters" && (
                      <div
                        style={{
                          fontSize: "0.95em",
                          color: "#1976d2",
                      ...
                      >
                        Please upload all your experience certificates as a
                        single PDF document.
                      </div>
                    )}
                  </label>
