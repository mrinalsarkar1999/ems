...
function EmployeeHome() {
    ...
    async function fetchOnboardingStatus() {
      ...
      try {
        const res = await fetch(
          getApiUrl(
            `/api/employee/onboarding-status?employeeId=${
              user.employeeId || ""
  ...
  useEffect(() => {
    async function fetchEmployeeRecord() {
      ...
      try {
        const res = await fetch(
          getApiUrl(
            `/api/employee/record?employeeId=${
              user.employeeId || ""
            }&username=${user.username || ""}`
          )
        );
  ...
  useEffect(() => {
    async function fetchUserInfo() {
      ...
      try {
        const res = await fetch(
          getApiUrl(`/api/employee/info?employeeId=${user.employeeId}`)
        );
        if (res.ok) {
          const data = await res.json();
          setUser(data);
  ...
  return (
    <div className="employee-home">
      {!loading && !onboarded && (
        <div
          className="onboarding-warning"
          style={{ color: "orange", margin: "16px 0", fontWeight: "bold" }}
        >
      ...
      <div className="profile-details">
        ...
        <div className="profile-grid">
          ...
          <div className="profile-field">
            ...
            <span>
              {user?.centerCode || user?.centreCode || (
        ...
        {employeeRecord && (
          <div className="onboarding-details">
            ...
            <table className="onboarding-table">
              <tbody>
                ...
                <tr>
                  ...
                  <td>
                    {employeeRecord.highestQualification || (
                      <span className="placeholder">Not provided</span>
                    )}
                  </td>
                </tr>
                ...
                {employeeRecord.emergencyContact &&
                  employeeRecord.emergencyContact.length > 0 &&
                  employeeRecord.emergencyContact.map((c, i) => (
                    <tr key={i}>
                      <th>{`Emergency Contact${
                        employeeRecord.emergencyContact.length > 1
                          ? " " + (i + 1)
                          : ""
                      }`}</th>
                      <td>
            ...
            {employeeRecord.documents &&
              employeeRecord.documents.length > 0 && (
