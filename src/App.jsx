...
const EmployeeForgotPassword = () => {
  ...
  const handleRequestOtp = async (e) => {
    ...
    try {
      const res = await fetch(
        "http://localhost:5000/api/employee/request-reset",
        {
          method: "POST",
      ...
      if (!res.ok) setError(data.error || "Failed to send OTP");
        ...
        setMessage("OTP sent to your registered email (simulated)");
        setStep(2);
      }
    } catch (err) {
  ...
  const handleVerifyReset = async (e) => {
    ...
    try {
      const res = await fetch(
        "http://localhost:5000/api/employee/verify-reset",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  return (
    <div className="login-bg">
      <form
        ...
        onSubmit={step === 1 ? handleRequestOtp : handleVerifyReset}
      >
        ...
        <button
          type="submit"
          className="login-btn bouncy-btn"
          disabled={loading}
        >
          {loading
            ? step === 1
...
const CentreForgotPassword = () => {
  ...
  const handleRequestOtp = async (e) => {
    ...
    try {
      const res = await fetch(
        "http://localhost:5000/api/centre/request-reset",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
...
function App() {
  ...
  const [showAdminRegistrationLinks, setShowAdminRegistrationLinks] =
    useState(false);
      ...
      subItems: [
        {
          ...
          path: "/attendance-tracking",
          description: "Present: 32, Absent: 3, Late: 2 (today)",
        },
        {
          text: "Attendance Reports",
          path: "/attendance-tracking",
          description: "Attendance Rate: 94% this month",
        },
      ],
    },
    {
  ...
  return (
    <Router>
      <div className="app-container">
        ...
        {sidebarOpen && (
          <div className="sidebar-overlay" onClick={handleDrawerToggle}></div>
        )}

        <nav className={`sidebar ${sidebarOpen ? "open" : ""}`}>{drawer}</nav>
          ...
          {userType === "admin" && showAdminRegistrationLinks ? (
            <div className="dashboard-container">
              ...
              <div style={{ textAlign: "center", marginTop: "2rem" }}>
                <button
                  ...
                  onClick={() => setShowAdminRegistrationLinks(false)}
                >
                  Back to Dashboard
                </button>
              </div>
            ...
            <Routes>
              ...
              <Route
                path="/leave-management"
                element={
                  <ProtectedRoute allowedUserTypes={["centre", "admin"]}>
                    <LeaveManagement />
                  </ProtectedRoute>
                }
              />
              ...
              />
              <Route
                path="/centre/forgot-password"
                element={<CentreForgotPassword />}
              />
            </Routes>
          )}
        </main>
